...
async function unsealUint64(signer: HardhatEthersSigner, handle: bigint): Promise<bigint> {
	await initCofhe(signer)
...
async function placeOrderWith(
	...
	const tx = await sigill.connect(buyer).placeOrder(encProductId, observer.address)
...
describe('Sigill — full E2E simulation', () => {
	...
	beforeEach(async () => {
		...
		for (const b of [buyer, buyer2]) {
			...
			await (await usdc.connect(b).approve(await cUSDC.getAddress(), WRAP_AMOUNT)).wait()
	...
	describe('registerObserver', () => {
		it('registers when bond is sufficient and emits event', async () => {
			...
			expect(await sigill.getObserverBondAmount(observer.address)).to.equal(BOND)
		...
		it('accumulates bond on repeat calls and pushes the observer twice', async () => {
			...
			await (await sigill.connect(observer).registerObserver({ value: BOND })).wait()
			await (await sigill.connect(observer).registerObserver({ value: BOND })).wait()

			expect(await sigill.getObserverBondAmount(observer.address)).to.equal(BOND * 2n)
			expect(await sigill.getObserversCount()).to.equal(2)
		})

		it('getObserverAt reverts on out-of-bounds index', async () => {
			await expect(sigill.getObserverAt(0)).to.be.revertedWith('Index out of bounds')
		})
	})
	...
	describe('fulfillOrder', () => {
		...
		it('marks the order Fulfilled, stores the AES key + CID, and pays the observer', async () => {
			...
			const aesRes = await cofhejs.unseal(BigInt(o.encAesKey), FheTypes.Uint128)
		...
		it('reverts on the second fulfillment of the same order', async () => {
			...
			await (
				await sigill.connect(observer).fulfillOrder(orderId, encAesKey1, 'cid-1')
			).wait()
	...
	describe('observer queue', () => {
		...
		it('tracks queue length and per-index order IDs across multiple orders', async () => {
			...
			expect(await sigill.getOrderQueue(observer.address)).to.deep.equal([0n, 1n, 2n])
	...
	describe('observer stats', () => {
		...
		it('updates getOrderCompleted and getOrderFailed across the lifecycle', async () => {
			...
			await (
				await sigill.connect(observer).rejectOrder(1, 'rejected')
			).wait()

			expect(await sigill.getOrderCompleted(observer.address)).to.equal(1)
			expect(await sigill.getOrderFailed(observer.address)).to.equal(0)
			expect(await sigill.getCompleteness(observer.address)).to.be.greaterThan(0)
		})
	})
	...
	describe('multiple observers', () => {
		it('registers and lists three independent observers', async () => {
			...
			expect(await sigill.getObserverBondAmount(observer.address)).to.equal(BOND)
			expect(await sigill.getObserverBondAmount(observer2.address)).to.equal(BOND * 2n)
			expect(await sigill.getObserverBondAmount(observer3.address)).to.equal(BOND * 3n)
		})

		it('keeps each observer queue independent', async () => {
			await (await sigill.connect(observer).registerObserver({ value: BOND })).wait()
			await (await sigill.connect(observer2).registerObserver({ value: BOND })).wait()
			...
			expect(await sigill.getOrderQueue(observer.address)).to.deep.equal([0n, 1n])
			expect(await sigill.getOrderQueue(observer2.address)).to.deep.equal([2n, 3n])
		...
		it('forbids cross-observer fulfillment and rejection', async () => {
			...
			await expect(
				sigill.connect(observer2).fulfillOrder(orderForObserver1, encAesKey, 'x')
			).to.be.revertedWith('Not observer')
		...
		it('tracks completion stats independently per observer', async () => {
			...
			await (await sigill.connect(observer).fulfillOrder(o1, k1, 'cid-1')).wait()
			await (await sigill.connect(observer).rejectOrder(o2, 'no')).wait()

			const k2 = await encryptUint128(observer2, 2n)
			await (await sigill.connect(observer2).fulfillOrder(o3, k2, 'cid-3')).wait()
			...
			expect(await sigill.getObserverBondAmount(observer.address)).to.equal(BOND)
			expect(await sigill.getObserverBondAmount(observer2.address)).to.equal(BOND)
		})
	...
	describe('ConfidentialERC20', () => {
		it('round-trips a wrap → unwrap', async () => {
			...
			expect(
				(await usdc.balanceOf(buyer.address)) - usdcBefore
		...
		it('rejects double-claim of the same unwrap', async () => {
			...
			await (await cUSDC.connect(buyer).claimUnwrap(unwrapId, 5_000_000n)).wait()
			await expect(
